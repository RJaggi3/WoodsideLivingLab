...
import pywt  # PyWavelets is needed for the wavelet transform
...
# Col 1: Frequency Domain (FFT of Original and Wavelet-Decimated)
# Col 2: Reconstructed Data (via wavelet reconstruction)
...
for channel_group in chunks(channels, 6):
    ...
    for i, channel in enumerate(channel_group):
        ...
        # ---------------------------------------------------
        # DECIMATION using Wavelet Transform (Data Reduction)
        # ---------------------------------------------------
        # Choose level = 1 which roughly halves the number of samples.
        level = 5
        mean_original = np.mean(data)
        data_demeaned = data - mean_original
        
        # Use pywt.wavedec to perform the decomposition.
        wavelet = 'db4'
        coeffs = pywt.wavedec(data_demeaned, wavelet, level=level)
        # The approximation coefficient vector (coeffs[0]) is our decimated data.
        data_decimated = coeffs[0] + mean_original
        # Create a corresponding time axis. Its length is len(data) divided by 2^level.
        channel_time_decimated = channel_time[::(2**level)]
        
        # ---------------------------------------------------
        # Wavelet Reconstruction:
        # Zero out detail coefficients and reconstruct the signal.
        reconstruction_coeffs = coeffs.copy()
        for j in range(1, len(reconstruction_coeffs)):
            reconstruction_coeffs[j] = np.zeros_like(reconstruction_coeffs[j])
        data_reconstructed = pywt.waverec(reconstruction_coeffs, wavelet) + mean_original
        
        # Trim (or pad) the reconstructed signal to ensure it matches the length of the original data.
        ...
        # Compute FFT on the decimated data.
        ...
        # ------------------------------------------------------------
        # RECONSTRUCTED SIGNAL (via wavelet reconstruction)
        # ------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # DIFFERENCE SIGNAL (Original - Reconstructed)
        # -------------------------------------------------------------
