...
from scipy.signal import decimate, find_peaks  # Note: now importing find_peaks
import statistics

def chunks(lst, n):
    for i in range(0, len(lst), n):
        yield lst[i:i + n]
d:\Tan\Monash\Year 4\ENG4701 - FYP Part 1\Significnat events\Earthquake\main_TN_simple_FFT_read_filter.py
# Define your TDMS file name and output PDF name
file_name = "202503281215_SHM-7.tdms"
pdf_name = file_name.replace(".tdms", "_FREQ_v3.2.pdf")
...
for channel_group in chunks(channels, 6):
    ...
    for i, channel in enumerate(channel_group):
        ...
        # --- DYNAMIC DECIMATION BASED ON FREQUENCY CONTENT ---
        dt = np.mean(np.diff(channel_time))  # estimated sampling interval
        Fs = 1 / dt  # original sampling frequency
        mean_original = np.mean(data)
        # Demean the data for frequency analysis
        data_demeaned = data - mean_original
        # Compute FFT on the original, demeaned data.
        fft_vals_full = np.fft.rfft(data_demeaned)
        freq_axis_full = np.fft.rfftfreq(len(data), d=dt)
        mag_full = np.abs(fft_vals_full)
        # Identify significant peaks in the magnitude spectrum.
        threshold = 0.1 * np.max(mag_full)
        peaks, properties = find_peaks(mag_full, height=threshold)
        if peaks.size > 0:
            max_significant_freq = np.max(freq_axis_full[peaks])
            # Compute the decimation factor so that the new Nyquist frequency (Fs/(2*q))
            # remains greater than the maximum significant frequency.
            q_dynamic = max(1, int(Fs / (2 * max_significant_freq)))
        else:
            q_dynamic = 1  # No significant peaks found, so no decimation
            
        print(f"Channel {channel.name}: Original samples = {len(data)}, "
              f"Dynamic decimation factor = {q_dynamic}")
        
        # Apply dynamic decimation
        data_decimated = decimate(data_demeaned, q_dynamic, ftype='iir', zero_phase=True) + mean_original
        channel_time_decimated = channel_time[::q_dynamic]
        ...
        print(f"Channel {channel.name}: Decimated samples = {decimated_length}, " 
        ...
        if abs(mean_decimated_val) < threshold_val:
        ...
        # Adjust x-axis tick formatting: use scientific notation if necessary.
