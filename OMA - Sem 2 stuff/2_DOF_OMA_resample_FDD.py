...
# 5. Full-rate FDD (reference), stabilization plot suppressed
...
for method in methods:
    ...
    for q in q_factors:
        print(f" q = {q}")
        freqs       = results[method][q]['Frequencies']
        dams        = results[method][q]['Damping']
        macs        = results[method][q]['MAC']
        mode_shapes = results[method][q]['ModeShapes']

        n_modes = np.asarray(freqs).flatten().size
        for i in range(n_modes):
            f_val = freqs.flatten()[i]
            z_val = dams.flatten()[i]
            mac   = macs[i]
            phi   = mode_shapes[:, i]
            print("   Mode", i+1,
                  "→ Frequency =", f_val,
...
plt.ylabel("MAC")
plt.title("FDD: MAC vs Compression Factor")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("fdd_mac_vs_q.png", dpi=300)
plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("fdd_mode2_freq_vs_q.png", dpi=300)
plt.show()
